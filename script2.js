...
let currentPage = 1;
const pageSize = 8;
/* PUBLIC API SAMPLE: https://api.spaceflightnewsapi.net/v4/articles/ Tidak perlu API Key */ async function fetchNews() {
...
function displayNews(articles) {
  ...
  articles.forEach((article) => {
    ...
    card.innerHTML = ` <img src="${article.image_url || "https://via.placeholder.com/400x250"}" alt="news"> <div class="news-content"> <h3>${article.title}</h3> <p>${article.summary.substring(0, 120)}...</p> <a href="${article.url}" target="_blank">Baca Selengkapnya →</a> </div> `;
